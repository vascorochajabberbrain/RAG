...
- **Git workflow (worktrees)**: When working in a Claude Code worktree, commit changes to the worktree branch and **push the branch to remote**. Do NOT merge into `main` or push `main` — the user will handle merging/pulling to `main` themselves. Process: commit → `git push -u origin <worktree-branch>` → user merges when ready.
