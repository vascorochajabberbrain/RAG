...
def _faq_extract_system_prompt(language: str, company_name: str, max_items: int) -> str:
    ...
    return (
        ...
        "Be GENEROUS in what you include — the operator reviews every\n"
        "pair before anything is written to the FAQ table, so prefer\n"
        "recall over precision. If you can find an answer that fits the\n"
        "question, include the pair even if the answer is short or you\n"
        "had to scan a few paragraphs after the question to find it.\n"
        "Rules:\n"
        "  - Output valid JSON: {\"pairs\": [{\"question\": \"...\", \"answer\": \"...\", \"category\": \"...\"}]}\n"
        "  - One pair per distinct topic.\n"
        "  - INCLUDE a pair whenever you can locate an answer for a\n"
        "    question — even a single sentence is fine. Only skip when\n"
        "    the answer is truly absent from the text.\n"
        "  - The answer can come from anywhere in the text (not just\n"
        "    the line immediately after the question). FAQ pages often\n"
        "    list all questions first and answers further down.\n"
        "  - Skip pure navigation blocks (\"Home\", \"Shop\", \"About\") and\n"
        "    repeated promotional banners. DO include genuine product\n"
        "    or policy info even if it appears in a sidebar or footer.\n"
